...
const DashboardContainer = () => {
  ...
  return (
    <div className={styles.dashboard_container}>
      <span style={{ fontSize: 48, color: "#C7493A" }}>
        Unmanaged Tournaments
      </span>
        ...
        >
          {data.unmanaged.length === 0 ? (
            <span style={{ color: "lightgray" }}>
              No new tournaments submitted yet
            </span>
          ) : (
            <>
                ...
                return (
                  <div
                    style={{
                      ...
                      color: "lightgray",
                      padding: 10,
                      marginBottom: index == data.unmanaged.length - 1 ? 0 : 10,
                    }}
                  >
                    <u>Name:</u> {value.name}, <u>Datetime:</u>{" "}
                    {d.toDateString()} {twoDigitFix(d.getUTCHours())}:
                    {twoDigitFix(d.getMinutes())} CET Time
                    <div>
                      <u>Author name:</u> {value.authorName}
                    </div>
                    {value.discord && (
                      <div>
                        <u>Discord:</u> {value.discord}
                      </div>
                    )}
                    {value.smashgg && (
                      <div>
                        <u>SmashGG:</u> {value.smashgg}
                      </div>
                    )}
                    {value.challonge && (
                      <div>
                        <u>Challonge:</u> {value.challonge}
                      </div>
                    )}
                    <a
                      style={{
                ...
                return (
                  <div
                    style={{
                      ...
                      color: "lightgray",
                      padding: 10,
                      marginBottom: index == data.all.length - 1 ? 0 : 10,
                    }}
                  >
                    <u>Name:</u> {value.name}, <u>Datetime:</u>{" "}
                    {d.toDateString()} {twoDigitFix(d.getUTCHours())}:
